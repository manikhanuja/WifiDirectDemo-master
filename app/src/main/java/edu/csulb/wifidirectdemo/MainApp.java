...
public class MainApp extends Application {
   ...
   public boolean mIsServer = false;
    ...
    public boolean isP2pEnabled() {
        String state = SystemPref.getStringFromPref(this, SystemPref.PREF_NAME, SystemPref.P2P_ENABLED);
    ...
    public String shiftInsertMessage(MessagingTuple row) {
        JSONObject jsonobj = MessagingTuple.getAsJSONObject(row);
        if( jsonobj != null ){
            mMessageArray.put(jsonobj);
        }
        mMessageArray = JSONUtils.truncateJSONArray(mMessageArray, 10);  // truncate the oldest 10.
